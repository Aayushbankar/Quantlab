...
\begin{itemize}
    \item \textbf{Brokerage:} Flat Rs. 20 per executed order.
...
\textbf{Answer:} We reject them through pessimistic Trade-Through logic. If we place a Buy Limit at Rs. 100, and the daily Low is exactly Rs. 100, we reject the fill. Without order book depth data, we must assume we were at the back of the queue and our lot was not executed. This prevents optimistic bias.
\end{defensebox}
